...
basics <- basics[!basics$titleType == "tvEpisode", ]
...
# write to CSV file
fwrite(basics, "movie_titles_metadata.csv")
...
dfplot %>%
  ...
  scale_y_continuous(limits = c(0,100000), breaks = seq(0, 100000, 10000), labels = label_comma()) +
  geom_bar(stat="identity") +
  xlab("Year") +
  ylab("")

# write to JPG file
ggsave("C:/Users/bge_j/Documents/UDS/Movie ecology/vis/yearly_releases.jpg", width = 7, height = 7)
...
# write to file
fwrite(crewcount, "crewcount.csv", sep = ";", row.names = F)
crewcount <- fread("crewcount.csv")
...
fwrite(df_gendersub, "crewgenders_sub.csv", sep = ";")

##### plot percentage of female crew members over time
library(reshape2)

df <- fread("crewgenders_sub.csv")
...
df_crew_n %>% group_by(tconst) 
df_crew_n <- df_crew_n %>% dplyr::count(tconst)
...
# write to CSV file
fwrite(df_crew_n, "crew_n.csv", sep=";")
...
# crew members do not include the director. 31959 out of 58444 will not list directors.
#------------------------------------------------------------------------------------------------
setwd("C:/Users/bge_j/Documents/UDS/Movie ecology/data/imdb")
...
fwrite(df_merged, "previous_mean_ratings.csv", sep=';')
